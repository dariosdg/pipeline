...
async function model() {
  ...
  for (const tenant of cfg.tenants.filter((t) => !wanted || t.slug === wanted)) {
    ...
    await tx(async (client) => {
      ...
      await client.query("INSERT INTO mart_daily_performance(tenant_id,day,gross_revenue,ad_spend,email_events) SELECT $1, days.day,coalesce(o.gross,0),coalesce(a.spend,0),coalesce(e.events,0) FROM (SELECT created_at::date AS day FROM stg_orders WHERE tenant_id=$1 UNION SELECT spend_date FROM stg_ad_spend WHERE tenant_id=$1 UNION SELECT occurred_at::date FROM stg_email_events WHERE tenant_id=$1) days LEFT JOIN (SELECT created_at::date day,sum(gross) gross FROM stg_orders WHERE tenant_id=$1 GROUP BY 1) o USING(day) LEFT JOIN (SELECT spend_date day,sum(spend) spend FROM stg_ad_spend WHERE tenant_id=$1 GROUP BY 1) a USING(day) LEFT JOIN (SELECT occurred_at::date day,count(*)::integer events FROM stg_email_events WHERE tenant_id=$1 GROUP BY 1) e USING(day)", [id]);
    });
